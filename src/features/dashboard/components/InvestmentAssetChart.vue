...
const monthlyHistory = computed(() => {
  ...
  const currentRate = clampRate(props.changeRate)
  const openingRate = clampRate(currentRate * 0.35)

  return Array.from({ length: 4 }, (_, index) => {
    const date = new Date(today.getFullYear(), today.getMonth() - (3 - index), 1)
    const progress = index / 3
    const wave = Math.sin(index * 1.4) * Math.min(0.6, currentRate * 0.06)
    return {
      date,
      rate: clampRate(openingRate + (currentRate - openingRate) * progress + wave),
    }
  })
})
...
<template>
  <section class="investment-chart">
    ...
    <div class="investment-chart__graph">
      <svg
        ref="chartElement"
        viewBox="0 0 313 100"
        role="img"
