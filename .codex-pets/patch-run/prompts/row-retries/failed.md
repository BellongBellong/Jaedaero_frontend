Create Codex pet row `failed` for `patch`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Tiny compact cyberpunk raccoon PR reviewer; charcoal-gray fur, bold raccoon mask and striped tail, cyan and warm-magenta tech accents, one small opaque wrist-mounted review terminal integrated into the left forearm, alert friendly expression, no readable text or logos.. Preserve silhouette, face, palette, material, proportions, markings, and props.
